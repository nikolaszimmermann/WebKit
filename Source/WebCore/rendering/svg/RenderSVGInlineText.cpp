...
static int offsetForPositionInFragment(const InlineIterator::SVGTextBox& textBox, const SVGTextFragment& fragment, float position)
{
    ...
    return fragment.characterOffset - textBox.start() + textBox.renderer().usedFont().offsetForPosition(textRun, position, includePartialGlyphs);
}
...
PositionWithAffinity RenderSVGInlineText::positionForPoint(const LayoutPoint& point, HitTestSource, const RenderFragmentContainer*)
{
    ...
    float baseline = m_usedFont.metricsOfPrimaryFont().ascent();
...
void RenderSVGInlineText::updateUsedFont()
{
    if (computeUsedFontForStyle(*this, style(), m_usedFont))
        m_canUseSimplifiedTextMeasuring = { };
}

bool RenderSVGInlineText::computeUsedFontForStyle(const RenderObject& renderer, const Style::ComputedStyle& style, FontCascade& usedFont)
{
    Ref<Document> document = renderer.document();

    // The font size may have been inherited from the HTML tree with CSS zoom already applied. The SVG
    // root transform scales the whole SVG subtree, so recompute the used size without zoom - the style
    // builder applies the same rule when font-size is specified on an SVG element.
    auto fontDescription = style.fontDescription();
    auto unzoomedFontSize = Style::usedFontSizeFromComputedSize(fontDescription.computedSize(), fontDescription.isAbsoluteSize(), true, style, document);

    // SVG controls its own glyph orientation, rather than letting writing-mode affect it.
    bool isVertical = fontDescription.orientation() != FontOrientation::Horizontal;
    bool needsGeometricPrecision = fontDescription.textRenderingMode() == TextRenderingMode::Auto;
    if (!isVertical && !needsGeometricPrecision && unzoomedFontSize.size == fontDescription.usedSize()) {
        usedFont = style.fontCascade();
        return false;
    }

    fontDescription.setUsedSize(unzoomedFontSize.size, unzoomedFontSize.zoomFactor);
    if (isVertical)
        fontDescription.setOrientation(FontOrientation::Horizontal);
    if (needsGeometricPrecision)
        fontDescription.setTextRenderingMode(TextRenderingMode::GeometricPrecision);

    usedFont = FontCascade(WTF::move(fontDescription));
    usedFont.update(protect(document->fontSelector()).ptr());
    return true;
}
